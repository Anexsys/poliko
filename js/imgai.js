...
const SKIP = ["Human", "People", "Person", "Crowd", "Face", "Female", "Male", "Portrait", "Man", "Woman", "Girl", "Boy"];
...
function detectLabels() {
    ...
    rekognition.detectLabels(params, function (err, data) {
        if (err) console.log(err, err.stack); // an error occurred
            ...
            if ($('#face-selection').hasClass('active')) {
                //Done with label output - detect faces
                detectFaces();
            }
            else if ($('#text-selection').hasClass('active')) {
                detectText();
            }
            else if ($('#scene-selection').hasClass('active')) {
                //print labels
                log("Found the following labels:\n");
                //var labelsWithoutBoring = labels.filter(function (x) { return !SKIP.includes(x); })

                labels.forEach(function (label) {
                    log("   " + label.Name + " (" + Math.round(label.Confidence) + "%)\n");
                });
                generateSceneSpeech();
            }
        }
...
function generateSceneSpeech() {
    var toSpeak = "";

    var text = "Hi, my name is Brian. In this picture I see the following objects: ";
    ...
    text += toSpeak;
    log("Synthesizing speech for top 5 objects with Polly:\n    \"" + text + "\"\n");
...
function generateFaceSpeech() {

    log("Identified with greater than 50% confidence:\n");


    var text = "Hi, my name is Brian. In this picture I see " + faces.FaceDetails.length + " face" + (faces.FaceDetails.length != 1 ? "s" : "") + ". ";
    ...
    log("Synthesizing speech for 3 first objects with Polly:\n    \"" + text + "\"\n");
...
function detectText() {
    ...
    rekognition.detectText(params, function (err, data) {
        if (err) console.log(err, err.stack); //an error occurred
            ...
            log("Found the following text:\n");
            var wordCount = 0;
            for (var i = 0; i < data.TextDetections.length; i++) {
                if (data.TextDetections[i].Type == "WORD") {
                    wordCount++;
                    var word = data.TextDetections[i];
                    words.push(word);
                    // draw a box around each word

                    var box = word.Geometry.BoundingBox;
                    ...
                    //context2d.fillText((wordCount), (box.Left * canvas.width) + 2, (box.Top * canvas.height) + fontSize);
                    context2d.restore();


                }
                if (data.TextDetections[i].Type == "LINE") {
                    var line = data.TextDetections[i]
                    log("   " + line.DetectedText + "\n");
                }
            }
